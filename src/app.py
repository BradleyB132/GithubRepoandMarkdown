...
def _dynamic_import(name: str, fallback: str) -> Any:
    """Try to import `name` (dotted) first, then `fallback` if that fails.
...
# Import consolidator, report and db modules in a way that works whether the
# package is executed as a package (from repo root) or as a script. Using
# importlib avoids duplicate top-level import statements that confuse static
# checkers like mypy.
_consolidator_mod = _dynamic_import("src.consolidator", "consolidator")
_report_mod = _dynamic_import("src.report", "report")
_db_mod = _dynamic_import("src.db", "db")

consolidate = getattr(_consolidator_mod, "consolidate")
summary_metrics = getattr(_report_mod, "summary_metrics")
high_severity_shipped = getattr(_report_mod, "high_severity_shipped")
get_engine = getattr(_db_mod, "get_engine")
fetch_source_rows = getattr(_db_mod, "fetch_source_rows")
...
def main() -> None:
    ...
    st.markdown(
        "Data is loaded from the configured database; use sidebar filters to refine the view."
    )
    ...
    try:
        ...
        st.success(
            f"Loaded {len(prod_rows)} production rows, {len(qual_rows)} quality rows, {len(ship_rows)} shipping rows from DB."
        )
    ...
    if consolidated:
        ...
        if "Production_Date" in df_con.columns:
            df_con["Production_Date"] = pd.to_datetime(
                df_con["Production_Date"], errors="coerce"
            )

        # Sidebar filters
        st.sidebar.header("Filters")
        line_options = sorted(
            {str(x) for x in df_con.get("Line_No", []).tolist() if x is not None}
        )
        if not line_options:
        ...
        if (
            "Production_Date" in df_con.columns
            and not df_con["Production_Date"].isna().all()
        ):
            ...
            date_val = st.sidebar.date_input(
                ...
                value=st.session_state.production_date_range,
                key="production_date_range",
            )
        ...
        if selected_lines:
            mask &= (
                df_con.get("Line_No", pd.Series([None] * len(df_con)))
                .astype(str)
                .isin(selected_lines)
            )
        if start_date and end_date and "Production_Date" in df_con.columns:
            mask &= df_con["Production_Date"].apply(
                lambda d: (d is not pd.NaT) and (start_date <= d.date() <= end_date)
            )
        ...
        if selected_severities:
            ...
            mask &= df_con.get("Severities", pd.Series([[]] * len(df_con))).apply(
                sev_match
            )
        ...
        if selected_defects:
            ...
            mask &= df_con.get("Defect_Types", pd.Series([[]] * len(df_con))).apply(
                defect_match
            )
        ...
        def shipped_flag(s):
            if not isinstance(s, dict):
                return False
            return bool(
                s.get("is_shipped") or s.get("Is_Shipped") or s.get("isShipped")
            )

        if shipped_choice == "Shipped":
            mask &= df_con.get("Shipping", pd.Series([None] * len(df_con))).apply(
                shipped_flag
            )
